      ...
      <div className="mx-auto max-w-7xl px-6">
        <div className="grid gap-6 md:grid-cols-2">
          ...
          >
            ...
            <div className="relative mt-10 h-48 w-full overflow-hidden rounded-xl opacity-70">
              <svg viewBox="0 0 340 200" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
                ...
                {[0,1,2,3,4,5,6].map((i) => {
                  ...
                  return (
                    <g key={i} opacity={op}>
                      <rect x="6" y={y + 3} width="16" height="16" rx="8" fill={i % 3 === 1 ? '#eac33e' : '#10b981'} opacity="0.3" />
                      <rect x="28" y={y + 5} width={w} height="7" rx="3.5" fill={color} filter="url(#blur1)" />
                      <rect x="28" y={y + 16} width={w * 0.55} height="4" rx="2" fill={color} opacity="0.4" filter="url(#blur1)" />
                      {[0,1,2].map((j) => (
                        <rect key={j} x={28 + w + 6 + j * 9} y={y + 4 + j * 3} width={3 - j * 0.5} height={3 - j * 0.5} rx="1" fill={i % 3 === 1 ? '#eac33e' : '#10b981'} opacity={0.25 - j * 0.07} />
                      ))}
                    </g>
                  );
                })}
            ...
            <div className="pointer-events-none absolute inset-0 opacity-[0.04]" style={{ backgroundImage: 'linear-gradient(rgba(16,185,129,0.6) 1px, transparent 1px), linear-gradient(90deg, rgba(16,185,129,0.6) 1px, transparent 1px)', backgroundSize: '32px 32px' }} />
          </article>
...
function Science() {
  ...
  return (
    <section id="science" className="section-glow relative py-28">
      <div aria-hidden className="pointer-events-none absolute inset-0" style={{ backgroundImage: 'radial-gradient(at 80% 20%, rgba(16,185,129,0.08), transparent 55%), radial-gradient(at 10% 90%, rgba(234,195,62,0.05), transparent 55%)' }} />
      <div className="relative mx-auto max-w-7xl px-6">
        <div className="grid grid-cols-1 gap-6 md:grid-cols-12 items-stretch">
          {/* Left — entry cards */}
          <div className="md:col-span-4 flex flex-col gap-4">
            <div className="card-glow rounded-[16px] p-5" style={{ background: 'linear-gradient(160deg, #0d1613 0%, #080c0b 100%)', border: '1px solid rgba(16,185,129,0.12)', boxShadow: 'var(--shadow-ambient)' }}>
              <span className="label-spaced text-[#eac33e]">Entry Category</span>
              <div className="mt-3 flex items-center gap-2">
                <span className="inline-block h-2.5 w-2.5 rounded-full bg-[#10b981]" style={{ boxShadow: '0 0 8px rgba(16,185,129,0.8)', animation: 'pulse-emerald 2s cubic-bezier(0.4,0,0.2,1) infinite' }} />
          ...
          {/* Right — color mapping */}
          <div className="card-glow md:col-span-8 rounded-[20px] p-8 md:p-10" style={{ background: 'linear-gradient(135deg, rgba(16,185,129,0.08), rgba(234,195,62,0.04))', border: '1px solid rgba(234,195,62,0.18)', boxShadow: 'var(--shadow-gold-glow)', backdropFilter: 'blur(20px)' }}>
            <h3 className="headline-tight text-2xl font-semibold text-emerald-50 md:text-3xl mb-8">Atmospheric Color Mapping</h3>
            <div className="grid grid-cols-5 gap-4">
              {colors.map(({ bg, shadow, label, tc }) => (
                <div key={label} className="card-glow group flex flex-col items-center gap-3">
                  <div className="w-full rounded-xl transition-all duration-300 group-hover:-translate-y-1 group-hover:scale-105" style={{ height: '48px', background: bg, boxShadow: `0 0 16px ${shadow}` }} />
                  <span className="label-spaced text-center" style={{ fontSize: '0.6rem', color: tc }}>{label}</span>
                </div>
              ))}
            </div>
          </div>
...
function Mission() {
  return (
    <section id="mission" className="section-glow relative py-28">
      <div aria-hidden className="pointer-events-none absolute inset-0" style={{ backgroundImage: 'radial-gradient(at 10% 50%, rgba(16,185,129,0.10), transparent 55%), radial-gradient(at 90% 20%, rgba(234,195,62,0.07), transparent 55%)' }} />
      <div className="relative mx-auto max-w-7xl px-6">
        <div className="grid grid-cols-1 items-center gap-16 md:grid-cols-2">
          ...
          <div>
            ...
            <div className="flex items-baseline gap-4 mb-8">
              <span className="text-glow text-[4.5rem] font-semibold leading-none tracking-[-0.04em] text-[#10b981]">32%</span>
              <p className="text-base leading-snug text-emerald-100/65 max-w-[200px]">Improvement in long-term retention over 2D interfaces.</p>
            </div>
            <p className="text-base leading-relaxed text-emerald-100/65 mb-10 max-w-lg">
              Our brains are biologically optimized for 3D environments. By mapping your journal entries to a persistent cityscape, Skyline leverages the{' '}
              <span className="font-bold text-[#eac33e]">&ldquo;Method of Loci&rdquo;</span>
              &mdash;a mnemonic system that uses spatial visualization to organize and recall information more effectively.
            </p>
          ...
          <div className="relative group">
            <div className="absolute -inset-4 rounded-[24px] blur-2xl opacity-30 group-hover:opacity-50 transition-opacity duration-1000" style={{ background: 'linear-gradient(135deg, rgba(234,195,62,0.25), rgba(16,185,129,0.20))' }} />
            <div className="card-glow relative overflow-hidden rounded-[20px]" style={{ background: '#0d1613', border: '1px solid rgba(16,185,129,0.12)', boxShadow: 'var(--shadow-ambient)' }}>
              <img
...
function CtaBand() {
  return (
    <section id="cta" className="section-glow relative py-32">
      <div aria-hidden className="pointer-events-none absolute inset-0" style={{ backgroundImage: 'radial-gradient(at 50% 50%, rgba(16,185,129,0.18), transparent 55%), radial-gradient(at 80% 30%, rgba(234,195,62,0.10), transparent 55%)' }} />
  ...
  return (
    <footer className="relative border-t border-[#10b981]/10 py-8">
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-center px-6">
        <a href="#" className="flex items-center gap-2">
          <img src="/skyline-logo.png" alt="Skyline" style={{ height: '28px', width: '28px', objectFit: 'contain', borderRadius: '5px', filter: 'drop-shadow(0 0 6px rgba(16,185,129,0.4))' }} />
